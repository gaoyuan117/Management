...
import static com.gaoyuan.management.util.Preconditions.checkNotNull;
...
public class BitmapCompress {
    ...
    private BitmapCompress(File cacheDir) {
        mCacheDir = cacheDir;
    }
    ...
    private static synchronized File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, BitmapCompress.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    public static BitmapCompress get(Context context) {
        mPathList = new ArrayList<>();

        if (INSTANCE == null) INSTANCE = new BitmapCompress(BitmapCompress.getPhotoCacheDir(context));
        return INSTANCE;
    }

    public BitmapCompress launch() {
        ...
        if (gear == BitmapCompress.FIRST_GEAR)
        ...
        else if (gear == BitmapCompress.THIRD_GEAR)
            Observable.fromIterable(mFile)
                    .map(new Function<File, File>() {
                        @Override
                        public File apply(File file) throws Exception {
    ...
    public BitmapCompress load(List<String> files) {
        mFile.clear();
    ...
    public BitmapCompress setCompressListener(OnCompressListener listener) {
        compressListener = listener;
        return this;
    }

    public BitmapCompress putGear(int gear) {
        this.gear = gear;
        return this;
    }
     ...
     * @deprecated
     */
    public BitmapCompress setFilename(String filename) {
    ...
    public Observable<File> asObservable() {
        if (gear == FIRST_GEAR)
            return Observable.fromIterable(mFile).map(new Function<File, File>() {
                @Override
                public File apply(File file) throws Exception {
                    return firstCompress(file);
                }
...
//            fos.write(stream.toByteArray());
    ...
    public interface OnCompressListener {
         ...
         */
        void onSuccess(List<String> picList);
